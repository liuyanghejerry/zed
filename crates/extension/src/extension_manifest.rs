    ...
    pub async fn load(fs: Arc<dyn Fs>, extension_dir: &Path) -> Result<Self> {
        ...
        if fs.is_file(&extension_manifest_path).await {
            let manifest_content = fs
                ...
                .await
                .with_context(|| format!("failed to load {extension_name} extension.json from {}", extension_manifest_path.display()))?;
            ...
            let manifest_content = fs
                .load(&extension_manifest_path)
                .await
                .with_context(|| format!("failed to load {extension_name} extension.toml from {}", extension_manifest_path.display()))?;
